            ...
            for move in correspondence:
                ...
                if move[1] == "CTO" or move[1] == "CVY":
                    ...
                    if len(move) <= 3: # if it is supporting a hold
                        ...
                        replacements.append((move[2], move[1], move[0], move[3], move[4]))
...
def orders_correspondence(orders: list) -> (list):
    ...
    for i, order in enumerate(orders):
        if order[1] == "SUP":
            ...
            convoy_moves = filter(lambda order: order[1] == "CTO" or order[1] == "CVY", orders)
...
def randomize(order: tuple) -> tuple:
    ...
    tag_to_func = {
        "WVE": {lambda: order},
        "BLD": random_build, "REM": random_build, "DSB": random_build, 
        "MTO": random_movement, "RTO": random_movement, 
        "HLD": random_hold,
        "SUP": random_support,
        "CVY": random_convoy,
