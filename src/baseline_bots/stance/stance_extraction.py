...
class ActionBasedStance(StanceExtraction):
    ...
    def extract_hostile_moves(self, nation, game_rec):
        ...
        for opp in self.nations:
            if opp == nation:
                continue
            if opp not in game_rec["orders"].keys():
                continue
    ...
    def get_stance(self, game_rec, message=None):
        ...
        for n in self.nations:
            (
                ...
                hostile_mov_to[n],
                conflit_mov_to[n],
            ) = self.extract_hostile_moves(n, game_rec)
        ...
        for n in self.nations:
            friendship_to[n], friendly_sup_to[n] = self.extract_friendly_supports(
                n, game_rec
            )

        self.stance = {
            n: {
                k: -hostililty_to[n][k] - hostililty_s_to[n][k] + friendship_to[n][k]
...
class ScoreBasedStance(StanceExtraction):
    ...
    def get_stance(self, game_rec, game_rec_type="JSON", message=None):
        """
        ...
        """
        # extract territory info
        if game_rec_type == "JSON":
            self.scores = self.extract_scores(game_rec)
        elif game_rec_type == "game":
            self.scores = game_rec
