...
class RLProposerBot(RLOrderBot):
    ...
    def __init__(
        ...
        self.K_ORDERS = 5
        super().load_model(
            "models/a2c_actor_diplomacy_proposer",
            "models/a2c_critic_diplomacy_proposer",
        )
    ...
    def gen_messages(self, rcvd_messages: List[Message]) -> MessagesData:
        ...
        for recipient in self.game.powers:
            proposal_list = []
            if (
                self.power_name != recipient
                and not self.game.powers[recipient].is_eliminated()
            ):
                ...
                for order in orders[: min(self.K_ORDERS, n)]:
                    ...
                    self.maa2c.env_state = dict_to_arr(
                        self.env.cur_obs, self.maa2c.n_agents
                    )
                    action = self.maa2c.exploration_action(self.maa2c.env_state)
                    action_dict = {
                        agent_id: action[agent_id]
                        if agent_id == self.env.power_mapping[self.power_name]
                        else 0
                        for agent_id in self.env.agent_id
                    }
