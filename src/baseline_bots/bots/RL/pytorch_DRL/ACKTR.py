...
class DisjointACKTR(A2C):
    ...
    def __init__(
        ...
        super(DisjointACKTR, self).__init__(
            env,
            state_dim,
            action_dim,
            memory_capacity,
    ...
    def train(self):
        ...
        if self.critic_optimizer.steps % self.critic_optimizer.Ts == 0:
            ...
            values_noise = to_tensor_var(
                np.random.randn(values.size()[0]), self.use_cuda
            )
            sample_values = (values + values_noise.view(-1, 1)).detach()
            if self.critic_loss == "huber":
                vf_fisher_loss = -self.vf_fisher_coef * nn.functional.smooth_l1_loss(
                    values, sample_values
                )
            else:
                vf_fisher_loss = -self.vf_fisher_coef * nn.MSELoss()(
                    values, sample_values
                )
...
class JointACKTR(A2C):
    ...
    def __init__(
        ...
        super(JointACKTR, self).__init__(
            env,
            state_dim,
            action_dim,
            memory_capacity,
        ...
        self.optimizer = KFACOptimizer(
            self.actor_critic, lr=min(self.actor_lr, self.critic_lr)
        )
