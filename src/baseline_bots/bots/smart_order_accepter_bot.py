...
class SmartOrderAccepterBot(DipnetBot):
        ...
        for proposer, orders in prp_orders.items():
            if (
                ...
                if proposer == best_proposer:
                    msg = YES(PRP(ORR(XDO(dipnet_to_daide_parsing(orders)))))
                else:
                    msg = REJ(PRP(ORR(XDO(dipnet_to_daide_parsing(orders)))))
                messages.add_message(proposer, str(msg))
        return messages

    def respond_to_invalid_orders(
        self, invalid_proposal_orders: Dict[str, List[str]], messages_data: MessagesData
    ) -> None:
        """
        ...
        for sender in invalid_proposal_orders:
            message = HUH(
                PRP(ORR(XDO(dipnet_to_daide_parsing(invalid_proposal_orders[sender]))))
            )
    ...
    def __call__(self, rcvd_messages: List[Tuple[int, Message]]):
        ...
        best_proposer, best_orders = get_best_orders(
