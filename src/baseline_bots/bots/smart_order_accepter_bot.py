...
class SmartOrderAccepterBot(DipnetBot):
    ...
    def __call__(self, rcvd_messages: List[Tuple[int, Message]]):
        ...
        # print(best_orders)
        # print(orders_data.get_list_of_orders())
        messages = self.gen_messages(orders_data.get_list_of_orders())

        # generate proposal response YES/NO to allies
        messages = self.gen_proposal_reply(best_proposer, valid_proposal_orders, messages)
