...
from baseline_bots.randomize_order import random_list_orders, string_to_tuple, tuple_to_string, randomize_order, ADJACENCY
...
class TestRandomizeDipnet:
    def test(self):
        ...
        assert valid_CVY((("FRA", "FLT", "NTH"),  'CVY', ("FRA", "AMY", "PIC"),"CTO",  "NWY" ))
        assert valid_CTO((("FRA", "AMY", "PIC"), "CTO", 'FIN', 'VIA' , ('NTH', 'SKA', 'DEN', 'BAL', 'BOT')))
...
def valid_unit(unit: Tuple) -> bool:
    ...
    vals =  isinstance(unit[0], str) and unit[0] in countries and isinstance(unit[1], str) and unit[1] in utypes and isinstance(unit[2], str) and unit[2] in ADJACENCY
    return types and vals

def valid_MTO(order: Tuple) -> bool:
    return order and valid_unit(order[0]) and order[2] and isinstance(order[2], str) and order[2] in ADJACENCY

def valid_SUP(order: Tuple) -> bool:
    valid_sup = order[0] and isinstance(order[0], Tuple) and order[1] and isinstance(order[1], str) and order[2] and isinstance(order[2], Tuple)
    valid_u1 = valid_unit(order[0])
    valid_u2 = valid_unit(order[2])
    if len(order) == 5:
        valid_ord = order[3] and order[3] == "MTO" and order[4] and order[4] in ADJACENCY
...
def valid_CVY(order: Tuple) -> bool:
    return valid_unit(order[0]) and valid_unit(order[2]) and len(order) == 5 and order[3] == 'CTO' and order[4] in ADJACENCY

def valid_CTO(order: Tuple) -> bool:
    return valid_unit(order[0]) and len(order) == 5 and order[2] in ADJACENCY and order[3] == 'VIA' and isinstance(order[4], Tuple)
