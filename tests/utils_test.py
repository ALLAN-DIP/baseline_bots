...
class TestUtils:
    ...
    def test(self):
        ...
        for tc_ip, tc_op in PARSING_TEST_CASES:
            assert (
                dipnet_to_daide_parsing(tc_ip, Game()) == tc_op
            ), dipnet_to_daide_parsing(tc_ip, Game())
            assert daide_to_dipnet_parsing(tc_op[0])[0] == tc_ip[0].replace(
                " R ", " - "
        ...
        for tc_ip, tc_op in PARSING_CVY_TEST_CASES:
            assert (
                dipnet_to_daide_parsing(tc_ip, game_tc) == tc_op
            ...
            for tc_ip_ord, tc_op_ord in zip(tc_ip, tc_op):
                assert daide_to_dipnet_parsing(tc_op_ord)[0] == tc_ip_ord.replace(
                    " R ", " - "
                ), daide_to_dipnet_parsing(tc_op_ord)
        ...
        for power_name, tc_ip, tc_op in PARSE_PROPOSALS_TC:
            ...
            parsed_orders_dict = parse_proposal_messages(
