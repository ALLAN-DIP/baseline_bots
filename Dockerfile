...
RUN apt-get -y update \
    && apt-get install --no-install-recommends -y git=1:2.17.1-1ubuntu0.16 \
    && apt-get clean \
...
RUN sed -i 's/gym>/gym=/g' research/requirements.txt \
    && pip install --no-cache-dir -r research/requirements.txt
...
# TODO: Fix this? https://github.com/hadolint/hadolint/wiki/DL3025
