...
for i in range(no_of_games):
	run(f"python dip_ui_bot_launcher.py -H shade.tacc.utexas.edu \
		-p RUSSIA -B rlspm -g kshenoy-{prefix}_test_{i} &", shell=True)
	time.sleep(10)
	run(f"python dip_ui_bot_launcher.py -H shade.tacc.utexas.edu \
		-p TURKEY -B rlsp -g kshenoy-{prefix}_test_{i} &", shell=True)
	time.sleep(20)
